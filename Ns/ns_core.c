...
void Ns_set_feature_extraction_parameters(NSinst_t* inst) {
...
// Initialize state
int Ns_InitCore(NSinst_t* inst, UWord32 fs) {
  int i;
  ...
  Ns_set_feature_extraction_parameters(inst); // Set feature configuration

  //default mode
  Ns_set_policy_core(inst, 0);
...
int Ns_set_policy_core(NSinst_t* inst, int mode) {
...
// Estimate noise
void Ns_NoiseEstimation(NSinst_t* inst, float* magn, float* noise) {
  int i, s, offset;
...
void Ns_FeatureParameterExtraction(NSinst_t* inst, int flag) {
...
void Ns_ComputeSpectralFlatness(NSinst_t* inst, float* magnIn) {
  int i;
...
void Ns_ComputeSpectralDifference(NSinst_t* inst, float* magnIn) {
...
int Ns_ProcessCore(NSinst_t* inst,
                         short* speechFrame,
                         short* speechFrameHB,
                         short* outFrame,
                         short* outFrameHB)
{
  ...
  if (inst->outLen == 0) {
    ...
    if (energy1 == 0.0) {
      ...
      for (i = 0; i < inst->blockLen10ms; i++) {
        dTmp = fout[i];
        if (dTmp < SPL_WORD16_MIN) {
          dTmp = SPL_WORD16_MIN;
        } else if (dTmp > SPL_WORD16_MAX) {
          dTmp = SPL_WORD16_MAX;
        }
    ...
    //compute spectral flatness on input spectrum
    Ns_ComputeSpectralFlatness(inst, magn);
    // quantile noise estimate
    Ns_NoiseEstimation(inst, magn, noise);
    ...
    Ns_ComputeSpectralDifference(inst, magn);
    ...
    if (updateParsFlag >= 1) {
      ...
      if (inst->modelUpdatePars[3] > 0) {
        Ns_FeatureParameterExtraction(inst, 0);
      }
      // compute model parameters
      if (inst->modelUpdatePars[3] == 0) {
        Ns_FeatureParameterExtraction(inst, 1);
        inst->modelUpdatePars[3] = inst->modelUpdatePars[1];
    ...
    // compute speech/noise probability
    Ns_SpeechNoiseProb(inst, probSpeechFinal, snrLocPrior, snrLocPost);
